...
%RTN edits to accommodate Noeske version RAEE 141007
            ...
            % RTN - read rhuser
            status = fseek(fid, 0, 'bof');
            [f_hdr_value, count] = fread(fid, 74+24, 'real*4'); % 2015
            ...
            % RTN 2015
            MRS_struct.p.voxsize(1) = f_hdr_value(74+22);
            MRS_struct.p.voxsize(2) = f_hdr_value(74+23);
            MRS_struct.p.voxsize(3) = f_hdr_value(74+24);
            ...
            if (nechoes == 1)
                ...
                MRS_struct.p.Navg(ii) = dataframes*navs*2; % RTN 2016
               
                if ((dataframes+refframes) ~= nframes)
                    mult = navs/2.0; % RTN 2016
                    multw = navs; % RTN 2016
                    noadd = 1;
                    dataframes = dataframes*navs;
                    refframes = nframes - dataframes; % refframes*navs; 2015
                else
                    mult = navs/2.0; % RTN 2016
                    multw = 1.0; % RTN 2016
                    noadd = 0;
                ...
                for loop = 1:refframes
                    WaterData(:,:,2*loop,:)=(-1)^(noadd*(loop-1))*ShapeData(:,:,1+loop,:) * multw; % RTN 2016
                    WaterData(:,:,2*loop-1,:)=(-1)^(noadd*(loop-1))*ShapeData(:,:,totalframes/2+1+loop,:) * multw; % RTN 2016
                end
                FullData = zeros([2 MRS_struct.p.npoints dataframes*2 nreceivers]);
                for loop = 1:dataframes
                    FullData(:,:,2*loop,:)=(-1)^(noadd*(loop-1))*ShapeData(:,:,1+refframes+loop,:) * mult; % RTN 2016
                    FullData(:,:,2*loop-1,:)=(-1)^(noadd*(loop-1))*ShapeData(:,:,totalframes/2+refframes+1+loop,:) * mult; % RTN 2016
                end
