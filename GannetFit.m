...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Gannet 2.1 version of Gannet Fit - analysis tool for GABA-edited MRS.
...
epsdirname = './MRSfit_output'; % MM (170121)
...
for ii=1:numscans
    ...
    if strcmp(MRS_struct.p.target,'GABA')
        ...
        lb = [-4000*maxinGABA -200 2.87 -40*maxinGABA -2000*maxinGABA]; %NP; our bounds are 0.03 less due to creatine shift
        ...
        % MM (170113)
        %pred = GaussModel_area(GaussModelParam(ii,:), freq(freqbounds));
        %obsv = real(GABAData(ii,freqbounds));
        %r = corr(pred(:), obsv(:));
        %MRS_struct.out.GABAFitR2(ii) = r^2;
        %MRS_struct.out.GABAFitMSE(ii) = MSE;
    ...
    elseif strcmp (MRS_struct.p.target,'GABAGlx')
        ...
        [GaussModelParam(ii,:),~,residg] = lsqcurvefit(@(xdummy,ydummy) GABAGlxModel_area(xdummy,ydummy), ...
            GaussModelInit, freq(freqbounds),real(GABAData(ii,freqbounds)), ...
            lb,ub,options);
        residg = -residg;
        if(fit_method == FIT_NLINFIT)
            ...
            for fit_iter = 1:100
                try % MM (170228)
                    ...
                    GaussModelParam(ii,:) = GaussModelInit;
                end
                MRS_struct.out.fitparams_iter(fit_iter,:,ii) = GaussModelParam(ii,:);
                GaussModelInit = GaussModelParam(ii,:);
                %ci = nlparci(GaussModelParam(ii,:), residg,'covar',COVB); %copied over
        ...
        residGABA = residg(residFreq <= 3.55 & residFreq >= 2.79);
        residGlx = residg(residFreq <= 4.10 & residFreq >= 3.45);
        
        GABAheight = GaussModelParam(ii,7);
        Glxheight = max(GaussModelParam(ii,[1,4]));
        % FitSTD reports the standard deviation of the residuals / gaba HEIGHT
        MRS_struct.out.GABAGlxFitError(ii) = 100*std(residg)/max([GABAheight, Glxheight]);
        MRS_struct.out.GABAFitError(ii) = 100*std(residGABA)/GABAheight;
        MRS_struct.out.GlxFitError(ii) = 100*std(residGlx)/Glxheight;
        ...
        MRS_struct.out.GABAArea(ii) = GaussModelParam(ii,7)./sqrt(-GaussModelParam(ii,8))*sqrt(pi);
        sigma = (1/(2 * (abs(GaussModelParam(ii,8))))).^(1/2);
        MRS_struct.out.GABAFWHM(ii) = abs((2*MRS_struct.p.LarmorFreq)*sigma);
        
        MRS_struct.out.GlxArea(ii) = (GaussModelParam(ii,1)./sqrt(-GaussModelParam(ii,2))*sqrt(pi)) + ...
            (GaussModelParam(ii,4)./sqrt(-GaussModelParam(ii,5))*sqrt(pi));
        sigma = ((1/(2*(abs(GaussModelParam(ii,2))))).^(1/2)) + ((1/(2*(abs(GaussModelParam(ii,5))))).^(1/2));
        MRS_struct.out.GlxFWHM(ii) = abs((2*MRS_struct.p.LarmorFreq)*sigma);
        
        MRS_struct.out.GABAGlxModelFit(ii,:) = GaussModelParam(ii,:);
        MRS_struct.out.GABAGlxresid(ii,:) = residg;
        MRS_struct.out.GABAresid(ii,:) = residGABA;
        MRS_struct.out.Glxresid(ii,:) = residGlx;
        %MRS_struct.out.GABAsnr(ii) = GABAheight / std(residg);
    ...
    elseif strcmp(MRS_struct.p.target,'GABAGlx')
        ...
        %determine values of GABA tail (below 2.8 ppm.
        hgabaglxres=text(2.8,min(residg),'residual');
    ...
    if strcmp(MRS_struct.p.Reference_compound,'H2O')
        ...
        if(waterfit_method == FIT_NLINFIT)
            ...
            %figure(8)
            [LGPModelParam(ii,:),residw,~,~,MSE] = nlinfit(freq(freqbounds), real(WaterData(ii,freqbounds)),...
        ...
        MRS_struct.out.WaterResid(ii,:) = residw; % MM (160913)
        ...
        % MM (170113)
        %pred = LorentzGaussModelP(LGPModelParam(ii,:), freq(freqbounds));
        %obsv = real(WaterData(ii,freqbounds));
        %r = corr(pred(:), obsv(:));
        %MRS_struct.out.WaterFitR2(ii) = r^2;
        %MRS_struct.out.WaterFitMSE(ii) = MSE;
        
        residw = residw + watmin - resmax;
        %stdevresidw=std(residw);
        if any(strcmp(MRS_struct.p.target,{'GABA','Glx'}))
            MRS_struct.out.GABAIU_Error_w(ii) = (MRS_struct.out.GABAFitError(ii).^2 + MRS_struct.out.WaterFitError(ii).^2).^0.5;
        elseif strcmp(MRS_struct.p.target,'GABAGlx')
            MRS_struct.out.GABAGlxIU_Error_w(ii) = sqrt(MRS_struct.out.GABAGlxFitError(ii).^2 + MRS_struct.out.WaterFitError(ii).^2);
            MRS_struct.out.GABAIU_Error_w(ii) = sqrt(MRS_struct.out.GABAFitError(ii).^2 + MRS_struct.out.WaterFitError(ii).^2);
            MRS_struct.out.GlxIU_Error_w(ii) = sqrt(MRS_struct.out.GlxFitError(ii).^2 + MRS_struct.out.WaterFitError(ii).^2);
        end
        
        % MM (170113)
        %z = fisherz(sqrt([MRS_struct.out.GABAFitR2(ii), MRS_struct.out.WaterFitR2(ii)]));
        %MRS_struct.out.GABA_WaterFitR2(ii) = ifisherz(mean(z))^2;
        %MRS_struct.out.GABA_WaterFitR2(ii) = ifisherz(sqrt(diff(z.^2)))^2;
        ...
        WaterArea(ii)=sum(real(LorentzGaussModel(LGPModelParam(ii,1:end-1),freq(freqbounds))) - BaselineModel(LGPModelParam(ii,3:5),freq(freqbounds)),2);
        % convert watersum to integral
        MRS_struct.out.WaterArea(ii)=WaterArea(ii) * (freq(1) - freq(2));
        %MRS_struct.H20 = GABA.nAvg(ii) ./ std(residw); %This line doesn't make sense - commenting pending delete. RE
        
        % MM (170202)
        LG = real(LorentzGaussModel(LGPModelParam(ii,1:end-1),freq(freqbounds))) - BaselineModel(LGPModelParam(ii,3:5),freq(freqbounds));
        LG = LG./max(LG);
        ind = find(LG >= 0.5);
        f = freq(freqbounds);
        w = abs(f(ind(1)) - f(ind(end)));
        MRS_struct.out.WaterFWHM(ii) = w * MRS_struct.p.LarmorFreq;
        
        % MM (160919)
        MRS_struct.spec.diff_scaled(ii,:) = MRS_struct.spec.diff(ii,:) .* (1/MRS_struct.out.WaterModelParam(ii,1));
        ...
        water_OFF = MRS_struct.spec.off(ii,:);
        ...
        waterbase = mean(real(MRS_struct.spec.off(ii,1:500)));
        ...
        LGPModelInit = lsqcurvefit(@LorentzGaussModelP, LGPModelInit, freq(freqWaterOFF), real(water_OFF), lblg, ublg, options);
        [MRS_struct.out.ResidWaterModelParam(ii,:), residRW] = nlinfit(freq(freqWaterOFF), real(water_OFF), @LorentzGaussModelP, LGPModelInit, nlinopts);
        MRS_struct.out.ResidWaterFitError(ii) = 100 * std(residRW) / MRS_struct.out.ResidWaterModelParam(ii,1);
        
        %figure(300);
        %model = LorentzGaussModelP(MRS_struct.out.ResidWaterModelParam(ii,:),freq(freqWaterOFF));
        %plot(freq(freqWaterOFF), water_OFF, 'b', freq(freqWaterOFF), model, 'r');
        %close(300)
        
        MRS_struct.out.WaterSuppressionFactor(ii) = (MRS_struct.out.WaterModelParam(ii,1) - abs(MRS_struct.out.ResidWaterModelParam(ii,1))) ...
            / MRS_struct.out.WaterModelParam(ii,1) * 100;
    ...
    elseif strcmp(MRS_struct.p.target,'GABAGlx')
        ...
        MRS_struct.out.GlxIU_Error_cr(ii) = sqrt(MRS_struct.out.GlxFitError(ii).^2 + MRS_struct.out.CrFitError(ii).^2);
    end
    ...
    %MRS_struct.out.GABA_CrFitR2(ii) = ifisherz(mean(z))^2;
    %MRS_struct.out.CrFitMSE(ii) = MSE;
    ...
    elseif strcmp(MRS_struct.p.target,'GABAGlx')
        MRS_struct.out.GABAconcCr(ii)=MRS_struct.out.GABAArea(ii)./MRS_struct.out.CrArea(ii);
        MRS_struct.out.GABAconcCho(ii)=MRS_struct.out.GABAArea(ii)./MRS_struct.out.ChoArea(ii);
        MRS_struct.out.GlxconcCr(ii)=MRS_struct.out.GlxArea(ii)./MRS_struct.out.CrArea(ii);
        MRS_struct.out.GlxconcCho(ii)=MRS_struct.out.GlxArea(ii)./MRS_struct.out.ChoArea(ii);
    end
    ...
    NAA_OFF = MRS_struct.spec.off(ii,:);
    % Initialise fitting parameters
    z = abs(MRS_struct.spec.freq-2.25); % MM: bound at 2.25 to avoid Glu/Gln-C4 resonances
    lowerbound = find(min(z)==z);
    z = abs(MRS_struct.spec.freq-1.75);
    upperbound = find(min(z)==z);
    freqrangeNAA = MRS_struct.spec.freq(lowerbound:upperbound);
    % MM: Using same fitting approach as for GABA/Glx
    maxinNAA = max(real(NAA_OFF(lowerbound:upperbound)));
    grad_points = (real(NAA_OFF(upperbound)) - real(NAA_OFF(lowerbound))) ./ (upperbound - lowerbound); %in points
    LinearInit = grad_points ./ (MRS_struct.spec.freq(1) - MRS_struct.spec.freq(2)); %in ppm
    constInit = (real(NAA_OFF(upperbound)) + real(NAA_OFF(lowerbound))) ./ 2;
    
    NAAModelInit = [maxinNAA 0.05 2.01 0 -LinearInit constInit];
    ...
    % MM: Fitting of Lorentzian model happens here
    NAAModelInit = lsqcurvefit(@LorentzModel, NAAModelInit, freqrangeNAA, real(NAA_OFF(lowerbound:upperbound)), lb, ub, options);
    % Refit to ensure convergence
    [NAAFitParams(ii,:), residNAA, ~, ~, MSE] = nlinfit (freqrangeNAA, real(NAA_OFF(lowerbound:upperbound)), ...
        @LorentzModel, NAAModelInit, nlinopts); %#ok<AGROW>
    
    % MM (170113)
    %pred = LorentzModel(NAAFitParams(ii,:), freqrangeNAA);
    %obsv = real(NAA_OFF(lowerbound:upperbound));
    %r = corr(pred(:), obsv(:));
    %MRS_struct.out.NAAFitR2(ii) = r^2;
    %MRS_struct.out.NAAFitMSE(ii) = MSE;
    
    NAAheight = NAAFitParams(ii,1)/(2*pi*NAAFitParams(ii,2));
    MRS_struct.out.NAAFitError(ii) = 100*std(residNAA)/NAAheight;
    %MRS_struct.out.GABAIU_Error_naa(ii) = (MRS_struct.out.GABAFitError(ii).^2 + MRS_struct.out.NAAFitError(ii).^2).^0.5;
    % MM: Still working on right way to calculate area - this seems to work (based on WaterArea)
    %MRS_struct.out.NAAArea(ii) = sum(real(LorentzModel(NAAFitParams(ii,:),freqrange)) - ...
        %BaselineModel([NAAFitParams(ii,3),NAAFitParams(ii,6),NAAFitParams(ii,5)],freqrange), 2) * ...
        %(freq(1) - freq(2));
    %MRS_struct.out.NAAArea(ii) = sum(real(LorentzModel(NAAFitParams(ii,:), freqrange) - ...
        %LorentzModel([0 NAAFitParams(ii,2:end)], freqrange))) * (freq(1) - freq(2)); % Taken from line 652
    %MRS_struct.out.NAAArea(ii) = NAAFitParams(ii,1); % MM: Peak area?
    MRS_struct.out.NAAFWHM(ii) = abs((2*MRS_struct.p.LarmorFreq) * NAAFitParams(ii,2));
    MRS_struct.out.NAAModelParam(ii,:) = NAAFitParams(ii,:);
    MRS_struct.out.NAAresid(ii,:) = residNAA;
    MRS_struct.out.NAASNR(ii) = abs(NAAheight) / noiseSigma_OFF;
    ...
    elseif strcmp (MRS_struct.p.target, 'GABAGlx')
        tmp = sprintf('GABA+/Glx Area : %.3g/%.3g', MRS_struct.out.GABAArea(ii), MRS_struct.out.GlxArea(ii));
    end
    text(0,0.6, tmp);
    if strcmp(MRS_struct.p.Reference_compound,'H2O')
        ...
        elseif strcmp (MRS_struct.p.target, 'GABAGlx')
            text(0,0.4, tmp, 'FontName', 'Helvetica');
            tmp = sprintf('GABA+/Glx (H2O)  : %.3f/%.3f inst. units.', MRS_struct.out.GABAconciu(ii), MRS_struct.out.Glxconciu(ii));
            text(0,0.3, tmp, 'FontName', 'Helvetica');
            tmp = sprintf('GABA+/Glx (Cr)  : %.3f/%.3f i.r.', MRS_struct.out.GABAconcCr(ii), MRS_struct.out.GlxconcCr(ii));
        end
...
EditingEfficiency_GABA = 0.5;
EditingEfficiency_Glx = 0.4;
...
switch MRS_struct.p.target
    case 'GABA'
        T1_factor_GABA = (1-exp(-TR./T1_Water)) ./ (1-exp(-TR./T1_GABA));
        T2_factor_GABA = exp(-TE./T2_Water) ./ exp(-TE./T2_GABA);
        
        if strcmpi(MRS_struct.p.vendor,'Siemens')
            MRS_struct.out.GABAconciu(ii) = (MRS_struct.out.GABAArea(ii)  ./  MRS_struct.out.WaterArea(ii))  ...
                * PureWaterConc*WaterVisibility*T1_factor_GABA*T2_factor_GABA*(N_H_Water./N_H_GABA) ...
                * MM /2.0./ EditingEfficiency_GABA; %Factor of 2.0 is appropriate for averaged data, read in separately as on and off (Siemens).
        else
            MRS_struct.out.GABAconciu(ii) = (MRS_struct.out.GABAArea(ii)  ./  MRS_struct.out.WaterArea(ii))  ...
                * PureWaterConc*WaterVisibility*T1_factor_GABA*T2_factor_GABA*(N_H_Water./N_H_GABA) ...
                * MM ./ EditingEfficiency_GABA;
        end
        
    case 'GABAGlx'
        T1_factor_GABA = (1-exp(-TR./T1_Water)) ./ (1-exp(-TR./T1_GABA));
        T2_factor_GABA = exp(-TE./T2_Water) ./ exp(-TE./T2_GABA);
        T1_factor_Glx = (1-exp(-TR./T1_Water)) ./ (1-exp(-TR./T1_Glx));
        T2_factor_Glx = exp(-TE./T2_Water) ./ exp(-TE./T2_Glx);
        
        if strcmpi(MRS_struct.p.vendor,'Siemens')
            MRS_struct.out.GABAconciu(ii) = (MRS_struct.out.GABAArea(ii)  ./  MRS_struct.out.WaterArea(ii))  ...
                * PureWaterConc * WaterVisibility * T1_factor_GABA * T2_factor_GABA * (N_H_Water./N_H_GABA) ...
                * MM ./ 2.0 ./ EditingEfficiency_GABA; %Factor of 2.0 is appropriate for averaged data, read in separately as on and off (Siemens).
            MRS_struct.out.Glxconciu(ii) = (MRS_struct.out.GlxArea(ii)  ./  MRS_struct.out.WaterArea(ii))  ...
                * PureWaterConc * WaterVisibility * T1_factor_Glx * T2_factor_Glx * (N_H_Water./N_H_Glx) ...
                ./ 2.0 ./ EditingEfficiency_Glx; %Factor of 2.0 is appropriate for averaged data, read in separately as on and off (Siemens).
        else
            MRS_struct.out.GABAconciu(ii) = (MRS_struct.out.GABAArea(ii)  ./  MRS_struct.out.WaterArea(ii))  ...
                * PureWaterConc * WaterVisibility * T1_factor_GABA * T2_factor_GABA * (N_H_Water./N_H_GABA) ...
                * MM ./ EditingEfficiency_GABA;
