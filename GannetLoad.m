...
%Gannet 2.1 GannetLoad
%Started by RAEE Nov 5, 2012
%Updates by MM 2016-2017
...
for ii=1:numpfiles    %Loop over all files in the batch (from gabafile)
    ...
    switch MRS_struct.p.vendor
        ...
        case 'Siemens_twix'
            ...
            % MM (160914)
            if isequal(MRS_struct.p.Water_Positive,0)
        ...
        case 'Siemens'
            ...
            % MM (160919): This is set further below
    ...
    %First steps
    % MM (160919): Set number zerofill points here to deal with cases where
    % spectral resolution differs across acquisitions
    ...
    freqrange=MRS_struct.p.sw/MRS_struct.p.LarmorFreq;
    MRS_struct.spec.freq=(MRS_struct.p.ZeroFillTo+1-(1:1:MRS_struct.p.ZeroFillTo))/MRS_struct.p.ZeroFillTo*freqrange+4.68-freqrange/2.0;
    MRS_struct.p.df=abs(MRS_struct.spec.freq(1)-MRS_struct.spec.freq(2)); % MM (160919)
    MRS_struct.p.spec_res=MRS_struct.p.sw/MRS_struct.p.npoints; % MM (170119)
    ...
    % MM (170201)
    water_range = MRS_struct.spec.freq-4.68 >= -0.5 & MRS_struct.spec.freq-4.68 <= 0.5;
    ...
    switch MRS_struct.p.vendor
        ...
        case 'Siemens_twix'
            AllFramesFTrealign=AllFramesFT;
    end %end of switch for Water max alignment pre-initialisation
    
    MRS_struct.fids.waterfreq(ii,:) = MRS_struct.spec.freq(FrameMaxPos);%to be used for the output figure
    
    % MM (170125)
    F0 = MRS_struct.spec.freq(FrameMaxPos);
    MRS_struct.out.AvgDeltaF0(ii) = mean(F0 - 4.68);
    ...
    % MM (170116): Determine how well SpecReg worked by calculating a similarity metric (squared Euclidean distance)
    MRS_struct.p.F0_preCorr(ii,:) = MRS_struct.fids.waterfreq(ii,:);
    FrameMax = max(abs(real(AllFramesFTrealign(water_range,:))),[],1);
    FrameMaxPos = zeros(1,size(AllFramesFTrealign,2));
    for kk = 1:size(AllFramesFTrealign,2)
        FrameMaxPos(kk) = find(abs(real(AllFramesFTrealign(:,kk))) == FrameMax(kk));
    end
    MRS_struct.p.F0_postCorr(ii,:) = MRS_struct.spec.freq(FrameMaxPos);
    alignedTo = ceil(numel(MRS_struct.p.F0_preCorr(ii,:))/10); % SpecReg aligns to 10% into acq.
    MRS_struct.p.F0_preCorr_d(ii) = euclid(MRS_struct.p.F0_preCorr(ii,:), alignedTo);
    MRS_struct.p.F0_postCorr_d(ii) = euclid(MRS_struct.p.F0_postCorr(ii,:), alignedTo);
...
%%%%%%%%%%%%%%% SIMILARITY METRIC %%%%%%%%%%%%%%%%%%%%%%
function d = euclid(x,n)
% Calculate a similarity metric based on squared Euclidean distance
