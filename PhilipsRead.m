...
% 110825
   ...
   MRS_struct.p.TR(MRS_struct.ii) = str2num(sparheader{sparidx+2}); % MM (170127)
   sparidx=find(ismember(sparheader, 'echo_time')==1); % MM (160805)
   MRS_struct.p.TE(MRS_struct.ii) = str2num(sparheader{sparidx+2}); % MM (160805)
   sparidx=find(ismember(sparheader, 'synthesizer_frequency')==1); % MM (160908)
   MRS_struct.p.LarmorFreq = str2num(sparheader{sparidx+2})/1e6;   % MM (160908)
   ...
   MRS_struct.p.Philips.SoftwareVersion = sparheader{sparidx+2};
   ...
   %for ii = 
    ...
    MRS_struct.p.voxdim(MRS_struct.ii,2) = str2num(sparheader{sparidx+2}); % MM (170127)
    sparidx=find(ismember(sparheader, 'lr_size')==1);
    MRS_struct.p.voxdim(MRS_struct.ii,1) = str2num(sparheader{sparidx+2});
    sparidx=find(ismember(sparheader, 'cc_size')==1);
    MRS_struct.p.voxdim(MRS_struct.ii,3) = str2num(sparheader{sparidx+2});
